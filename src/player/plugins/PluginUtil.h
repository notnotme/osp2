...
// Maps a possibly-null C string (the decoder libraries leave absent fields as nullptr) to a
// std::string.
...
// Reads a whole file into a byte vector; nullopt when the file cannot be opened. Deliberately does
// NOT catch: the read can throw (bad_alloc on a large file under the Switch's constrained RAM), and
// each caller keeps its own must-not-throw try block — which also covers its library calls — and
// its own SDL_Log prefix.
