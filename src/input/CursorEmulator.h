...
// A = left click, X = right click. Platform layer — main-thread-only, called once per frame from the
// render loop between ImGui_ImplSDL2_NewFrame() and ImGui::NewFrame() (the ImGui IO injection point).
...
public:
    ...
    // Reads the pad, integrates the left-stick deflection into the cursor position, and injects the
    // cursor position and button state into ImGui via io.AddMouse*Event.
