...
- `getStatus()` returns a `PlaybackStatus` (`src/player/PlaybackStatus.h`) snapshot — state, title, filename, position, duration — built under a **single** `m_mutex` lock. The snapshot construction lives in the private `statusLocked(stateOverride)` (precondition: caller holds the non-recursive `m_mutex` and `m_activePlugin` is non-null — it reads the plugin directly rather than calling the re-locking getters); `getStatus()` stamps the current `m_state`, and a `STOPPED` state zeroes position/duration per the "0 when stopped" contract. The plugin virtuals `getPosition()`/`getDuration()` (seconds) read the shared decoder, so they are contractually **only** called under `m_mutex`. Position/duration are `0` when stopped or unknown.
...
- **Reload continuity (no "No track" flash across a file boundary).** When `play(path)` replaces a track that is *currently being presented* (next/prev rolling past the first/last subsong into the adjacent file), the synchronous teardown above would flash the player bar to STOPPED/empty for the async-load window — a visible blink. Instead `play(path)` detects the reload (`m_activePlugin != nullptr`, **or** `m_reloadActive` already set — so continuity survives *chained* advances, e.g. next pressed twice quickly while a load is in flight, when `m_activePlugin` is already null but a prior snapshot is still valid), snapshots the outgoing track's `PlaybackStatus` into `m_reloadStatus` (via the same `statusLocked` helper `getStatus()` uses, stamped `PLAYING` so nothing is zeroed), sets `m_reloadActive`, and **keeps `m_state`/`m_currentPath`** (only the plugin is closed and nulled — the audio-thread contract still requires the plugin being opened to be inactive; a `PLAYING` state with a null plugin is fine, `decode()` checks both and outputs silence). While `m_reloadActive` is set, `getStatus()` returns `m_reloadStatus` at the top, so the bar keeps the outgoing title/highlight/metadata until `update()` swaps the replacement in. `isReloading()` (a locked read of `m_reloadActive`) exposes the window so `Application` **gates its metadata refetch** on `!isReloading()`: the outgoing plugin is closed, so `getCurrentSubtrack()` reads its real `0` (which `advance()` needs) even though the bar still shows the outgoing track — refetching then would blank the Metadata tab (`getMetadata()` returns empty with no active plugin), so the outgoing path/subtrack/metadata are held until swap-in. `getSubtrackCount()`/`getCurrentSubtrack()` themselves deliberately do **not** honor the snapshot (a subtrack step during a reload must not appear valid). `m_reloadActive` is cleared on swap-in **success**; on **failure/cancel/stop** the player reverts to a genuine STOPPED "No track" (`update()`'s failure and cancel branches, and `stop()`, all clear `m_reloadActive` and `m_state = STOPPED` / `m_currentPath.clear()` — a first play from a stopped state keeps today's immediate STOPPED teardown, since nothing is being presented). The decode **"Loading…" overlay is reserved for direct clicks and slow/remote loads**: `Application` suppresses it for boundary/auto advances via `m_advanceLoadInFlight` (set when it starts a `play()` with `m_advanceDirection != 0`, cleared when the play result is consumed or work is cancelled), so `makeUiState()` computes `loading = m_player.isLoading() && !m_advanceLoadInFlight`. A **remote** sibling still shows the "Downloading…" fetch overlay during the FTP fetch (that is `fsWorking`, unaffected). See [application.md](application.md).
...
- **Plugins are RAII**: each takes the output sample rate as a constructor parameter and fully tears down (decoder object, worker threads, library shutdown) in its destructor — without calling the virtual `close()`. `PlayerController::create()` constructs them directly (`std::make_unique<XxxPlugin>(SAMPLE_RATE)`); `destroy()` closes the audio device first, `close()`s each plugin, then `m_plugins.clear()` runs the destructors — so a destructor can never race `decode()`.
