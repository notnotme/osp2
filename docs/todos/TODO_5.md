...
- [x] **5b — Metadata tab UI**: `drawFileMetadata` visit dispatch (no generic fallback), `drawModuleMetadata` field table + scrollable message region. Verify play/stop/auto-advance refresh the tab, and run the temporary dummy-alternative compile check.
