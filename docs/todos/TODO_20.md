...
- [x] **20b — CJK-capable font**: add a subset CJK font under `romfs/font/`, merge it in `Platform::loadFonts()` with the appropriate glyph ranges (Japanese at minimum; consider full CJK if size allows), and verify the atlas builds on both platforms. Mind the `romfs/` size on Switch — subset aggressively. Verify on Switch hardware (ask before deploying): the Shift-JIS NSF title now renders as Japanese glyphs, not tofu; Latin and icon glyphs still render; startup/atlas build time and memory stay acceptable.
