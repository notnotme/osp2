...
- [ ] **9c — SidPlugin** (gated on prerequisite **P1**). `src/player/plugins/SidPlugin.{h,cpp}` over libsidplayfp (`sidplayfp` engine + `SidTune` + `ReSIDfpBuilder`, forward-declared), 16-bit→float, extensions `sid, psid, rsid`, default subtune = tune's start song. `SidMetadata` (from `SidTuneInfo`: title/author/released + SID model + clock) + `drawSidMetadata`. Registration + `pkg_check_modules(libsidplayfp …)` stanza + `add_executable` entry. Use built-in ReSIDfp defaults — **no external C64 ROM files**.
