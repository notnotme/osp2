...
- [x] **9e — Sc68Plugin** (gated on prerequisite **P2**). `src/player/plugins/Sc68Plugin.{h,cpp}` over libsc68 (`sc68_create`/`sc68_load`/`sc68_process`, forward-declared), 16-bit→float, extensions `sc68, sndh, snd`, default subtune = disc default track. `Sc68Metadata` (title/author/composer/hardware/ripper) + `drawSc68Metadata`. Registration + `pkg_check_modules(sc68 …)` stanza + `add_executable` entry.
