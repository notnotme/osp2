...
- [x] **7c — Robustness + polish**: "Scanning..." vs "Downloading..." overlay label, cache-path sanitization (+ `.`/`..` traversal guard), timeout tuning (stall 30→15 s), a **Cancel button** on the overlay to abort a stuck scan/download (curl `XFERINFOFUNCTION` + atomic flag; cancelled download doesn't auto-advance; cancelled scan stays put), fetch-failure UX (auto-advance skips the broken sibling; direct click just logs), docs pass. **Switch hardware verified** (nxlink netloader): browses/plays Modland over Wi-Fi. Desktop verified: cancel aborts a stuck scan/download and stays put/doesn't chain; labels correct; both builds green.
